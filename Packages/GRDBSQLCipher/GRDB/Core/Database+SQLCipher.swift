...
extension Database {
    ...
    /// Sets the passphrase used by an attached SQLCipher database.
    ///
    /// This project keeps SQLCipher keys as `Data`, so plaintext-to-encrypted
    /// export needs the same byte-oriented API for attached databases.
    public func usePassphrase(_ passphrase: Data, forAttachedDatabase databaseName: String) throws {
        let code = databaseName.withCString { databaseNamePointer in
            passphrase.withUnsafeBytes {
                sqlite3_key_v2(sqliteConnection, databaseNamePointer, $0.baseAddress, CInt($0.count))
            }
        }
        guard code == SQLITE_OK else {
            throw DatabaseError(resultCode: code, message: String(cString: sqlite3_errmsg(sqliteConnection)))

...
> 当前做到：账号模块 - 退出登录与账号切换完成，下一步验证账号数据删除入口。
...
| 已完成 | 第 2 周 | 退出登录与账号切换 | 会话列表账号入口，清除登录态后回到登录页，重新登录其他账号重建依赖容器 | 退出登录后不展示原账号数据；切换账号后进入新账号会话列表且不串目录 | iOS |
...
> 本轮验证（2026-05-06）：视频相关 5 项定向单测通过；`AppleIMTests` 全量回归仍有非视频失败，涉及 ChatViewModel 分页/语音播放、会话列表刷新、FTS 修复、网络恢复协调，视频相关用例均通过。

...
| 部分完成 | 第 5 周 | 文本消息发送 | `async/await` 请求服务端，ack 回写；服务端适配层、DTO 映射、baseURL/token provider 与 `TokenRefreshActor` 已就绪，真实 endpoint 待接入 | 本地/适配层 success/failed 状态正确；真实接口联调后补齐端到端验收 | iOS/Server |

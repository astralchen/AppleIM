...
| 部分完成 | 第 14 周 | 回归测试 | iOS 15+ 机型和模拟器覆盖 | iOS 26.4.1 Simulator 对照关键 UI 路径与 `AppleIMTests` 全量通过；iOS 15 真机/旧系统矩阵仍需 QA 设备补跑 | QA |
...
> 进度更新（2026-05-11）：稳定回归复测与修复完成。11 个历史超时用例集合在 iPhone 17 / iOS 26.4.1 Simulator 通过，`xcodebuild ... -only-testing:AppleIMTests` 首轮复测不再复现 11 个超时，但暴露 `conversationListCellPrepareForReuseClearsAvatarImage` 间歇失败；已修复 `ConversationListCell.prepareForReuse()`，递归重置 UIKit 包装层内的 `ConversationListCellContentView` 并清理旧 `contentConfiguration`，避免复用时保留上一条头像。修复后 `conversationListCellPrepareForReuseClearsAvatarImage`、11 个历史超时用例集合、`xcodebuild -project AppleIM.xcodeproj -scheme AppleIM -destination 'platform=iOS Simulator,name=iPhone 17' test -only-testing:AppleIMTests`、`xcodebuild -project AppleIM.xcodeproj -scheme AppleIM -destination 'generic/platform=iOS Simulator' build-for-testing` 均通过。当前剩余回归风险收敛为 iOS 15 真机/旧系统矩阵待 QA 设备补跑，以及真实服务端文本 ack、重发、媒体上传、撤回/删除多端一致性联调。

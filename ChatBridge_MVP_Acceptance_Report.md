...
| 文本消息 | 待联调 | 本地入库与 sending 状态已完成；真实发送 ack 与失败状态待服务端验证。 |
| 消息重发 | 待联调 | 本地幂等模型和 pending job 基础已具备；真实接口重试不重复插入待验证。 |
...
| 风险 | 当前影响 | 后续动作 |
|---|---|---|
| 服务端文本消息 ack 未完成联调 | 无法确认真实 success/failed 状态回写 | 接口就绪后补充端到端发送、失败、重试验收。 |
...
- 本地 Mock/MVP：可进入内测。
- 真实服务端联调版本：需完成文本 ack、重发、媒体上传、撤回/删除同步验证后再进入灰度。
